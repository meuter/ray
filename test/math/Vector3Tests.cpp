...
#define TEST_VECTOR3_COORDINATES(vec, scalar, coord1, coord2, coord3, s1, s2, s3) \
    TEST(vec, coord_##coord1##_IsScalar) { auto v = vec{s1,s2,s3}; EXPECT_EQ(typeid(v.coord1), typeid(scalar)); }\
    TEST(vec, coord_##coord2##_IsScalar) { auto v = vec{s1,s2,s3}; EXPECT_EQ(typeid(v.coord2), typeid(scalar)); }\
    TEST(vec, coord_##coord3##_IsScalar) { auto v = vec{s1,s2,s3}; EXPECT_EQ(typeid(v.coord3), typeid(scalar)); }\
